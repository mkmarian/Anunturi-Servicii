<x-app-layout>
    <div class="py-8">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Editează cerere</h1>
                <p class="text-sm text-gray-500 mt-1 truncate">{{ $cerere->title }}</p>
            </div>
            <div class="bg-white shadow-sm rounded-2xl border border-gray-100 p-6">
                <form method="POST" action="{{ route('customer.requests.update', $cerere) }}">
                    @csrf @method('PUT')
                    @include('account.requests._form')

                    <div class="mt-6 flex justify-end gap-3">
                        <a href="{{ route('customer.requests.index') }}"
                           class="px-4 py-2 text-sm text-gray-700 border border-gray-200 rounded-xl hover:bg-gray-50 transition">
                            Anulează
                        </a>
                        <x-primary-button>Salvează modificările</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</x-app-layout>
